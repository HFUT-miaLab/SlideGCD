...
from model.abmil_GraphBuffer_InfoNCE_Reg_MFA import BClassifier
...
def train(model, loader, criterion_main, criterion_dual, optimizer_warmup, scheduler_warmup, optimizer, scheduler,
    ...
    for step, (feat, label) in enumerate(loader):
        ...
        if args.current_epoch < args.warmup_epochs:
            ...
            logits, logits_graph, buffer_update_loss, reg_loss = model(feat, label)
            ...
            graph_loss = (criterion_main(logits_graph, label) + args.distill_loss_weight * js_div +
                          args.buffer_update_weight * buffer_update_loss + args.reg_weight * reg_loss)
...
def test(weights, loader, args):
    milnet = BClassifier(args.feats_size, len(args.classes), args.k, args.buffer_size, args.batch_size, args.contrastive_temperature).cuda()
...
def main():
    ...
    parser.add_argument('--warmup_epochs', default=25, type=int, help='Number of total training epochs')
    parser.add_argument('--gpu_index', type=int, nargs='+', default=(0,), help='GPU ID(s) [0]')
    parser.add_argument('--batch_size', default=64, type=int, help='Number of total training epochs')
    parser.add_argument('--num_workers', default=4, type=int, help='Number of total training epochs')
    ...
    parser.add_argument('--distill_loss_weight', default=4, type=float)
    parser.add_argument('--contrastive_temperature', default=0.5, type=float)
    parser.add_argument('--buffer_update_weight', default=1, type=float)
