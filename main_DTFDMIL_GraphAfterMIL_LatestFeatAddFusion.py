...
from model.DTFD_MIL.DTFDMIL_GraphBuffer_FIFO_MFA_LatestFeatAddFusion import DTFD_MIL
...
def train(model, loader, criterion_main, criterion_dual, optimizer_warmup, scheduler_warmup, optimizer, scheduler,
    ...
    total_main_loss, total_fusion_loss = 0, 0
    for step, (feat, label) in enumerate(loader):
        optimizer.zero_grad()
        optimizer_warmup.zero_grad()

        feat, label = feat.cuda(), label.cuda()
        logits, sub_preds, logits_fusion = model(feat)
        ...
        if args.current_epoch < args.warmup_epochs:
            ...
            js_div = (criterion_dual(torch.log_softmax(logits_fusion / args.distill_temperature, dim=1),
                      ...
                      criterion_dual(torch.log_softmax(logits.detach() / args.distill_temperature, dim=1),
                                     torch.softmax(logits_fusion / args.distill_temperature, dim=1)))
            fusion_loss = criterion_main(logits_fusion, label) + args.distill_loss_weight * js_div

        loss = main_loss + fusion_loss

        correct += (torch.argmax(logits, dim=1) == label).sum().item()
        correct_graph += (torch.argmax(logits_fusion, dim=1) == label).sum().item()
        ...
        total_fusion_loss += fusion_loss

    return total_main_loss / total, total_fusion_loss / total, correct / total, correct_graph / total
