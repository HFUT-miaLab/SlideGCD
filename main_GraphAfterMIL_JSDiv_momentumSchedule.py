...
from model.abmil_GraphBuffer_FIFO_MFAv2 import BClassifier
...
def train(model, loader, criterion_main, criterion_dual, optimizer_warmup, scheduler_warmup, optimizer, scheduler,
    ...
    for step, (feat, label) in enumerate(loader):
        ...
        if args.model == 'dsmil':
            ...
            it = args.current_epoch * args.niter_per_ep + step
            distill_loss_weight = args.distill_weight_schedule[it]
            ...
            if args.current_epoch < args.warmup_epochs:
                ...
                graph_ce_loss = criterion_main(logits_graph, label)

            loss = main_loss + graph_ce_loss + distill_loss_weight * js_div
        else:
        ...
        total_graph_loss += graph_ce_loss + distill_loss_weight * js_div
...
def test(weights, loader, args):
    if args.model == 'abmil':
        milnet = BClassifier(args.feats_size, len(args.classes), k=args.k,
                             buffer_size=args.buffer_size, dropout=args.dropout).cuda()
...
def main():
    ...
    parser.add_argument('--distill_loss_weight', default=2.0, type=float)
    ...
    for fold in range(5):
        ...
        args.niter_per_ep = math.ceil(len(train_ids) / args.batch_size)
        args.distill_weight_schedule = util.cosine_scheduler(base_value=2, final_value=0.1, epochs=args.num_epochs,
                                                             niter_per_ep=args.niter_per_ep,
                                                             warmup_epochs=args.warmup_epochs, start_warmup_value=1)
