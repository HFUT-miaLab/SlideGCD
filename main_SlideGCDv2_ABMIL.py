...
from model.ABMIL.SlideGCDv2_ABMIL import SlideGCD_ABMIL
...
def train(model, loader, criterion_main, criterion_dual, optimizer_warmup, scheduler_warmup, optimizer, scheduler,
    ...
    correct_mlp, correct_graph, total = 0, 0, 0
    ...
    for step, (feat, label) in enumerate(loader):
        ...
        if args.current_epoch < args.warmup_epochs:  # warmup stage
            logits_mlp = model(feat)

            main_loss = criterion_main(logits_mlp, label)
            graph_loss = 0
        else:
            logits_mlp, logits_graph, buffer_update_loss, reg_loss = model(feat, label)

            main_loss = criterion_main(logits_mlp, label)

            js_div = (criterion_dual(torch.log_softmax(logits_graph / args.distill_temperature, dim=1), torch.softmax(logits_mlp.detach() / args.distill_temperature, dim=1)) +
                      criterion_dual(torch.log_softmax(logits_mlp.detach() / args.distill_temperature, dim=1), torch.softmax(logits_graph / args.distill_temperature, dim=1)))
            graph_loss = (criterion_main(logits_graph, label) + args.distill_loss_weight * js_div + args.buffer_update_weight * buffer_update_loss + args.reg_weight * reg_loss)

            correct_graph += (torch.argmax(logits_graph, dim=1) == label).sum().item()

        loss = main_loss + graph_loss
        correct_mlp += (torch.argmax(logits_mlp, dim=1) == label).sum().item()
        loss.backward()
    ...
    return total_main_loss / total, total_graph_loss / total, correct_mlp / total, correct_graph / total
...
def test(weights, loader, args):
    milnet = SlideGCD_ABMIL(args.feats_size, len(args.classes), args.k, args.buffer_size, args.batch_size, args.contrastive_temperature).cuda()
    ...
    with torch.no_grad():
        for step, (feat, label) in enumerate(loader):
            feat, label = torch.squeeze(feat).cuda(), torch.squeeze(label).cuda()
            logits, logits_graph = milnet(feat)
...
def main():
    ...
    parser.add_argument('--num_epochs', default=2, type=int, help='Number of total training epochs')
    parser.add_argument('--warmup_epochs', default=1, type=int, help='Number of total training epochs')
    parser.add_argument('--device', type=str, default='cuda:0')
    parser.add_argument('--batch_size', default=64, type=int)
    parser.add_argument('--num_workers', default=8, type=int)
    parser.add_argument('--weight_decay', default=5e-3, type=float, help='Weight decay [5e-3]')
    parser.add_argument('--task', default='TCGA-BRCA', type=str,
                        choices=['TCGA-NSCLC', 'TCGA-BRCA'], help='Dataset folder name')
    ...
    parser.add_argument('--reg_weight', default=0.25, type=float)
    args = parser.parse_args()

    if args.task == 'TCGA-NSCLC':
        args.classes = ['LUAD', 'LUSC']
    elif args.task == 'TCGA-BRCA':
        args.classes = ['IDC', 'ILC']
