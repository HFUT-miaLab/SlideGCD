...
def train(model, loader, criterion, optimizer, scheduler, args):
    ...
    for step, (feat, label) in enumerate(loader):
        ...
        if args.model == 'transmil':
            bag_prediction = model(data=feat)['logits']
        ...
        # print(torch.argmax(bag_prediction.view(feat.shape[0], -1), dim=1), label)
        # print((torch.argmax(bag_prediction.view(feat.shape[0], -1), dim=1) == label).sum().item())
...
def test(weights, loader, args):
    if args.model == 'abmil':
        ...
        milnet = TransMIL(len(args.classes)).cuda()
        milnet.load_state_dict(weights)
    else:
        raise NotImplementedError
    ...
    with torch.no_grad():
        for step, (feat, label) in enumerate(loader):
            feat, label = feat.cuda(), label.cuda()
            if args.model == 'transmil':
                bag_prediction = milnet(data=feat)['logits']
            elif args.model == 'abmil':
...
def main():
    ...
    parser.add_argument('--lr', default=0.00002, type=float, help='Initial learning rate [0.0002]')
