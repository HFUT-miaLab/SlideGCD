...
class SETMIL(nn.Module):
    def __init__(self, img_size=224, tokens_type='performer', in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=nn.LayerNorm, token_dim=64, irpe=None, token_t=None,
                 channel_reduce_rate=3, aspp_flag=1):
        super().__init__()
        ...
        if self.aspp_flag:
            self.aspp = ASPP(img_size=img_size // (channel_reduce_rate // 2), in_chans=token_dim, embed_dim=embed_dim,
                             **token_t)
        ...
        self.pos_embed = nn.Parameter(data=get_sinusoid_encoding(n_position=num_patches + 1, d_hid=decoder_embed_dim),
                                      requires_grad=False)
