...
class DTFD_MIL(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        if self.training:
            if y is None:  # Warmup训练阶段
                ...
                buffer_update_loss, reg_loss = self.buffer_update(x, y, e=self.e)
    ...
    def buffer_update(self, x, y, e=0.2):
        ...
        cls_center_sim_matrix = 1 + torch.cosine_similarity(cls_means.unsqueeze(0), cls_means.unsqueeze(1), dim=-1) - torch.eye(self.num_classes).cuda()
        reg_term = torch.sum(cls_center_sim_matrix) / 2 / (self.num_classes * (self.num_classes - 1) / 2)
        ...
        return pos_loss + neg_loss, reg_term
...
if __name__ == '__main__':
    ...
    similarity_matrix = 1 + torch.cosine_similarity(cls_means.unsqueeze(0), cls_means.unsqueeze(1), dim=-1) - 2 * torch.eye(rehearsal.shape[0])
    print(similarity_matrix)
    print(torch.sum(similarity_matrix) / 2, (rehearsal.shape[0] * (rehearsal.shape[0] - 1) / 2))
    print(torch.sum(similarity_matrix) / 2 / (rehearsal.shape[0] * (rehearsal.shape[0] - 1) / 2))
    # print(torch.mean(1 + similarity_matrix) / 2)
    # print(torch.cosine_similarity(cls_means.unsqueeze(0), cls_means.unsqueeze(1), dim=-1))
    # reg_term = torch.mean(1 + torch.cosine_similarity(cls_means.unsqueeze(0), cls_means.unsqueeze(1), dim=-1))
