...
from model.PatchGCN.SlideGCD_PatchGCN import SlideGCD_PatchGCN
...
def train(model, loader, criterion_main, criterion_dual, optimizer_warmup, scheduler_warmup, optimizer, scheduler,
    ...
    for step, G in enumerate(loader):
        ...
        logits_mlp, logits_graph = model(data=G)
        ...
        if args.current_epoch < args.warmup_epochs:
            ...
            js_div = (criterion_dual(torch.log_softmax(logits_graph / args.distill_temperature, dim=1),
                                     torch.softmax(logits_mlp.detach() / args.distill_temperature, dim=1)) +
                      criterion_dual(torch.log_softmax(logits_mlp.detach() / args.distill_temperature, dim=1),
...
def eval(model, loader, args):
    ...
    with torch.no_grad():
        for step, G in enumerate(loader):
            G, label = G.cuda(), G.y
            logits, logits_graph = model(data=G)
...
def test(weights, loader, args):
    milnet = SlideGCD_PatchGCN(args.feats_size, len(args.classes), args.k, args.buffer_size, args.batch_size).cuda()
    ...
    with torch.no_grad():
        for step, G in enumerate(loader):
            G, label = G.cuda(), G.y
            logits, logits_graph = milnet(data=G)
...
def main():
    ...
    parser.add_argument('--trainval_csv',
                        default=r'E:\WorkGroup\st\Project_ContinualLearning\GraphLearning\data\tcga_brca_trainval_fold(PatchGCN).csv', type=str)
    parser.add_argument('--test_csv',
                        default=r'E:\WorkGroup\st\Project_ContinualLearning\GraphLearning\data\tcga_brca_test(PatchGCN).csv', type=str)
