...
from model.DTFDMIL.SlideGCD_DTFDMIL import SlideGCD_DTFDMIL
...
def train(model, loader, criterion_main, criterion_dual, optimizer_warmup, scheduler_warmup, optimizer, scheduler,
    ...
    for step, (feat, label) in enumerate(loader):
        ...
        logits_mlp, sub_preds, logits_graph = model(feat)

        slide_sub_labels = [label for i in range(args.num_group)]
        slide_sub_labels = torch.cat(slide_sub_labels, dim=0)  ### numGroup
        main_loss = criterion_main(sub_preds, slide_sub_labels) + criterion_main(logits_mlp, label)
...
def test(weights, loader, args):
    milnet = SlideGCD_DTFDMIL(args.feats_size, len(args.classes), args.k, args.buffer_size, args.batch_size,
