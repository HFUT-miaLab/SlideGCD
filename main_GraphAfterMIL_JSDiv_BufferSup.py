...
from model.abmil_GraphBuffer_FIFO_MultiscaleFetureAttn_LabelQueue import BClassifier
...
def train(model, loader, criterion_main, criterion_dual, optimizer_warmup, scheduler_warmup, optimizer, scheduler,
    ...
    for step, (feat, label) in enumerate(loader):
        ...
        if args.model == 'dsmil':
            ...
            logits, logits_graph, node_labels = model(feat, labels=label)
            batch_logits_graph = logits_graph[:feat.shape[0], :]
            main_loss = criterion_main(logits.view(feat.shape[0], -1), label)
            if args.current_epoch < args.warmup_epochs:
                graph_loss = 0
            else:
                js_div = (criterion_dual(torch.log_softmax(batch_logits_graph / args.distill_temperature, dim=1),
                          ...
                          criterion_dual(torch.log_softmax(logits.detach() / args.distill_temperature, dim=1),
                                         torch.softmax(batch_logits_graph / args.distill_temperature, dim=1)))
                graph_loss = criterion_main(logits_graph, node_labels) + args.distill_loss_weight * js_div
        ...
        correct_graph += (torch.argmax(batch_logits_graph.view(feat.shape[0], -1), dim=1) == label).sum().item()
        total += label.shape[0]
    ...
    with torch.no_grad():
        for step, (feat, label) in enumerate(loader):
            ...
            if args.model == 'dsmil':
                ...
                logits, logits_graph, _ = model(feat, train=False)
                logits_graph = logits_graph[:feat.shape[0], :]
            else:
                raise NotImplementedError
...
def test(weights, loader, args):
    if args.model == 'abmil':
        milnet = BClassifier(args.feats_size, len(args.classes), k=args.k, buffer_size=args.buffer_size).cuda()
    ...
    with torch.no_grad():
        for step, (feat, label) in enumerate(loader):
            ...
            if args.model == 'dsmil':
                ...
                logits, logits_graph, _ = milnet(feat, train=False)
                logits_graph = logits_graph[:feat.shape[0], :]
            else:
                raise NotImplementedError
...
def main():
    ...
    parser.add_argument('--k', default=4, type=int, help='Number of neighbor in graph generated by DSL')
    parser.add_argument('--distill_temperature', default=2.0, type=float)
    parser.add_argument('--distill_loss_weight', default=2, type=float)
    ...
    for fold in range(5):
        ...
        optimizer = torch.optim.Adam(milnet.parameters(), lr=0.01 * args.lr * math.sqrt(args.batch_size),
                                     betas=(0.5, 0.9), weight_decay=args.weight_decay)
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, args.num_epochs - args.warmup_epochs,
                                                               0.000005)
