...
from model.TransMIL_GraphBuffer_FIFO_MFAv3 import TransMIL
...
def train(model, loader, criterion_main, criterion_dual, optimizer_warmup, scheduler_warmup, optimizer, scheduler, args):
    ...
    for step, (feat, label) in enumerate(loader):
        ...
        if args.model == 'transmil':
            logits, logits_graph, logits_distill = model(feat)
            main_loss = criterion_main(logits.view(feat.shape[0], -1), label)
            if args.current_epoch < args.warmup_epochs:
                graph_loss = 0
            else:
                js_div = (criterion_dual(torch.log_softmax(logits_distill / args.distill_temperature, dim=1),
                          ...
                          criterion_dual(torch.log_softmax(logits.detach() / args.distill_temperature, dim=1),
                                         torch.softmax(logits_distill / args.distill_temperature, dim=1)))
...
def eval(model, loader, args):
    ...
    with torch.no_grad():
        for step, (feat, label) in enumerate(loader):
            feat, label = feat.cuda(), label.cuda()
            if args.model == 'transmil':
                logits, logits_graph, _ = model(feat)
            else:
                raise NotImplementedError
...
def test(weights, loader, args):
    if args.model == 'transmil':
        milnet = TransMIL(args.feats_size, len(args.classes), k=args.k, buffer_size=args.buffer_size).cuda()
    ...
    with torch.no_grad():
        for step, (feat, label) in enumerate(loader):
            feat, label = feat.cuda(), label.cuda()
            if args.model == 'transmil':
                logits, logits_graph, _ = milnet(feat)
            else:
                raise NotImplementedError
...
def main():
    ...
    parser.add_argument('--buffer_size', default=3072, type=int)
    parser.add_argument('--k', default=14, type=int, help='Number of neighbor in graph generated by DSL')
