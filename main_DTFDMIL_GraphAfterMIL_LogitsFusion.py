...
from model.DTFD_MIL.DTFDMIL_GraphBuffer_FIFO_MFA import DTFD_MIL
...
def train(model, loader, criterion_main, criterion_dual, optimizer_warmup, scheduler_warmup, optimizer, scheduler,
    ...
    for step, (feat, label) in enumerate(loader):
        ...
        logits, sub_preds, logits_graph = model(feat)
        ...
        if args.current_epoch < args.warmup_epochs:
            ...
            graph_loss = criterion_main(logits_graph + logits, label) + criterion_main(logits_graph, label)
...
def eval(model, loader, args):
    ...
    targets, scores, scores_graph, scores_fusion = [], [], [], []
    with torch.no_grad():
        for step, (feat, label) in enumerate(loader):
            ...
            scores_fusion.extend(torch.softmax(logits + logits_graph, dim=-1).squeeze().cpu().numpy().tolist())

    targets, scores, scores_graph, scores_fusion = np.array(targets), np.array(scores), np.array(
        scores_graph), np.array(scores_fusion)
    if args.task != 'TCGA-Merged':
        ...
        acc_fusion, macro_auc_fusion, _, _, _, _, _ = metric.MetricEvaluator(args.classes).update(targets,
                                                                                                  scores_fusion)
    else:
        ...
        acc_fusion, macro_auc_fusion, _, _ = metric.MetricEvaluator(args.classes).update(targets, scores_fusion)

    return acc, macro_auc, acc_graph, macro_auc_graph, acc_fusion, macro_auc_fusion


def test(weights, loader, args):
    milnet = DTFD_MIL(args.feats_size, len(args.classes), args.k, args.buffer_size,
    ...
    targets, scores, scores_graph, scores_fusion = np.asarray(targets), np.asarray(scores), np.asarray(
        scores_graph), np.asarray(scores_fusion)

    return targets, scores, scores_graph, scores_fusion
...
def main():
    ...
    parser.add_argument('--buffer_size', default=4096, type=int, help='Number of total training epochs')
    ...
    parser.add_argument('--distill_loss_weight', default=1.0, type=float)
    ...
    fusion_evaluator = metric.MetricEvaluator(classes=args.classes, num_fold=5)
    for fold in range(5):
        ...
        best_model_saver_fusion = util.BestModelSaver(args.num_epochs, ratio=0)
        for epoch in range(args.num_epochs):
            ...
            valid_acc, valid_auc, valid_acc_graph, valid_auc_graph, valid_acc_fusion, valid_auc_fusion = eval(milnet,
                                                                                                              val_loader,
                                                                                                              args)
            best_model_saver.update(valid_acc, valid_auc, epoch)
            best_model_saver_graph.update(valid_acc_graph, valid_auc_graph, epoch)
            best_model_saver_fusion.update(valid_acc_fusion, valid_auc_fusion, epoch)
            print('\t\tEpoch: {} || lr: {:.6f} || train_acc (Main): {:.4f} || train_acc (Graph): {:.4f} || '
                  'train_loss (Main): {:.4f} || train_loss (Graph): {:.4f} || total_loss: {:.4f} || '
                  'valid_acc: {:.4f} || valid_acc(Graph): {:.4f} || valid_acc(Fusion): {:.4f} || '
                  'valid_auc: {:.4f} || valid_auc(Graph): {:.4f} || valid_auc(Fusion): {:.4f}'
                  .format(epoch, current_lr, train_acc, train_acc_graph, main_loss, graph_loss, main_loss + graph_loss,
                          valid_acc, valid_acc_graph, valid_acc_fusion, valid_auc, valid_auc_graph, valid_auc_fusion))
        ...
        shutil.copyfile(
            os.path.join(args.fold_save_path, 'epoch' + str(best_model_saver_fusion.best_valid_acc_epoch) + '.pth'),
            os.path.join(args.fold_save_path, 'best_acc(Fusion).pth'))
        ...
        targets, scores, _, _ = test(best_model_weight, test_loader, args)
        best_model_weight_graph = torch.load(os.path.join(args.fold_save_path, 'best_acc(Graph).pth'))
        targets, _, scores_graph, _ = test(best_model_weight_graph, test_loader, args)
        best_model_weight_graph = torch.load(os.path.join(args.fold_save_path, 'best_acc(Fusion).pth'))
        targets, _, _, scores_fusion = test(best_model_weight_graph, test_loader, args)
        ...
        if args.task != 'TCGA-Merged':
            ...
            acc_graph, macro_auc_graph, micro_auc_graph, f1_graph, _, _, _ = graph_evaluator.update(targets,
                                                                                                    scores_graph,
                                                                                                    fold=fold)
            acc_fusion, macro_auc_fusion, micro_auc_fusion, f1_fusion, _, _, _ = fusion_evaluator.update(targets,
                                                                                                         scores_fusion,
                                                                                                         fold=fold)
        else:
            ...
            acc_graph, macro_auc_graph, micro_auc_graph, f1_graph = graph_evaluator.update(targets, scores_graph,
                                                                                           fold=fold)
            acc_fusion, macro_auc_fusion, micro_auc_fusion, f1_fusion = fusion_evaluator.update(targets,
                                                                                                scores_fusion,
                                                                                                fold=fold)
        ...
        fusion_evaluator.plot_roc_curve(fold,
                                        save_path=os.path.join(args.fold_save_path,
                                                               'Plot_ROC_Curve(Fusion_Branch).jpg'))
        fusion_evaluator.plot_confusion_matrix(fold, save_path=os.path.join(args.fold_save_path,
                                                                            'Plot_Confusion_Matrix(Fusion_Branch).jpg'))
        ...
        print("\t\tBest_ACC_Model(Fusion): ACC: {:.4f}, Macro_AUC: {:.4f}, Macro_F1: {:.4f}"
              .format(acc_fusion, macro_auc_fusion, f1_fusion))
    ...
    ffv_metrics_fusion = fusion_evaluator.summary_acc() + fusion_evaluator.summary_macro_auc() + fusion_evaluator.summary_f1()
    print("\tBest_ACC_Model(Fusion): ACC: {:.2f}±{:.2f}, Macro_AUC: {:.2f}±{:.2f}, Macro_F1: {:.2f}±{:.2f}"
          .format(ffv_metrics_fusion[0] * 100, ffv_metrics_fusion[1] * 100, ffv_metrics_fusion[2] * 100,
                  ffv_metrics_fusion[3] * 100, ffv_metrics_fusion[4] * 100, ffv_metrics_fusion[5] * 100))
    fusion_evaluator.plot_all_roc_curve(
        save_path=os.path.join(args.weights_save_path, 'Plot_Whole_ROC_Curve(Graph).jpg'))
