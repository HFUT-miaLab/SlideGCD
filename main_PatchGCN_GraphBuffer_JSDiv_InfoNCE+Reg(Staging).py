...
from model.PatchGCN.PatchGCN_GraphBuffer_InfoNCE_Reg_MFA import PatchGCN
...
def train(model, loader, criterion_main, criterion_dual, optimizer_warmup, scheduler_warmup, optimizer, scheduler,
    ...
    for step, G in enumerate(loader):
        ...
        if args.current_epoch < args.warmup_epochs:
            ...
            logits_mlp, logits_graph, buffer_update_loss, reg_loss = model(G, label)
            ...
            graph_loss = criterion_main(logits_graph, label) + args.distill_loss_weight * js_div + args.reg_weight * reg_loss

            correct_graph += (torch.argmax(logits_graph, dim=1) == label).sum().item()

        loss = main_loss + graph_loss
        correct += (torch.argmax(logits_mlp, dim=1) == label).sum().item()
...
def test(weights, loader, args):
    milnet = PatchGCN(feat_dim=args.feats_size, num_classes=len(args.classes), k=args.k,
                      buffer_size=args.buffer_size, batch_size=args.batch_size, temp=args.contrastive_temperature).cuda()
...
def main():
    ...
    parser.add_argument('--trainval_csv', default='data/tcga_nsclc_staging_trainval_fold(PatchGCN).csv', type=str)
    parser.add_argument('--test_csv', default='data/tcga_nsclc_staging_test.csv', type=str)
    ...
    parser.add_argument('--distill_temperature', default=1.25, type=float)
    parser.add_argument('--distill_loss_weight', default=0.75, type=float)
    ...
    for fold in range(5):
        ...
        best_model_saver = util.BestModelSaver(args.num_epochs, ratio=0.3)
        best_model_saver_graph = util.BestModelSaver(args.num_epochs, ratio=0.3)
